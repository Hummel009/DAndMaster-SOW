...
После установки среды весь необходимый инструментарий готов к работе. Вот самые важные команды:

* Генерация запусков: `Меню Gradle -> Tasks -> forgegradle runs -> genIntelliJRuns`. После выполнения команды в верхней части окна появится кнопка запуска клиента игры.
